...
class BasePublicationRepository(ABC):
    ...
    @abstractmethod
    def get_publications_by_author_metadata(self, author_metadata: AuthorMetadata, max_results: int = 20) -> List[PaperMetadata]:
        """
    ...
    def get_publications(self, orcid: str, author_metadata: AuthorMetadata, max_results: int = 20) -> List[PaperMetadata]:
        """
        ...
        if self.SUPPORTS_ORCID_SEARCH:
            return self.get_publications_by_orcid(orcid, max_results)
        return self.get_publications_by_author_metadata(author_metadata, max_results)