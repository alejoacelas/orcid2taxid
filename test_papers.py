        ...
        for idx, orcid in enumerate(TEST_ORCIDS[:sample_size]):
            try:
                ...
                for pub in formatted_response:
                    ...
                    # Check external IDs
                    if pub.external_ids:
                        for id_type, ext_id in pub.external_ids.items():
                            assert ext_id.value and ext_id.value.strip(), \
                                f"Empty value for external ID type: {id_type}"
                            if ext_id.url:
                                assert ext_id.url.startswith(('http://', 'https://')), \
                                    f"Invalid URL for external ID: {ext_id.url}"
                    
                    # Check full text URLs
                    if pub.full_text_urls:
                        assert all(url.startswith(('http://', 'https://')) for url in pub.full_text_urls), \
                            "Invalid full text URL format"
                    
                    # Check funding information
                    if pub.funding_info:
                        for funding in pub.funding_info:
                            assert funding.funder or funding.grant_number or funding.name, \
                                "Empty funding information"
...
def test_author_metadata_search(sample_size: int = 5) -> TestSuiteResult:
    ...
    try:
        for idx, orcid in enumerate(TEST_ORCIDS[:sample_size]):
            try:
                author_metadata = orcid_client.get_author_metadata(orcid)
                
                # Get responses
                raw_response = repo.fetch_publications_by_author_metadata(author_metadata)
                formatted_response = repo.get_publications_by_author_metadata(author_metadata)
